...
def sos_run(workflow=None, targets=None, shared=None, args=None, source=None, **kwargs):
    ...
    try:
        ...
        if env.sos_dict.get('__concurrent_subworkflow__', False):
            ...
            env.logger.warning(f'Running nested workflow {", ".join(workflows)} in the middle of a step might lead to a deadlock and should be avoided.')
        res = {}
        for wf in wfs:
...
def downloadURL(URL, dest, decompress=False, index=None):
    ...
    try:
        ...
        prog.set_description(
            message + f':\033[32m downloaded{decompress_msg} {" "*(term_width - len(message) - 13 - len(decompress_msg))}\033[0m')
        prog.update()
        prog.close()
...
#
    ...
    if ret != 0:
        ...
        cmd = interpolate(f'pandoc {args}', {'input': sos_targets(
            temp_file), 'output': sos_targets(output)})
        raise RuntimeError(
